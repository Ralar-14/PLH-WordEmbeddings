...
def sentence_one_hot(sentence: str, dict: Dictionary, keys:List = None, preprocess: Callable = preprocess) -> np.ndarray:
...
def map_one_hot(sentence_pairs: List[Tuple[str, str, float]], dictionary:Dictionary, keys: List = None) -> List[Tuple[Tuple[np.ndarray, np.ndarray], float]]:
    pairs_vectors = []
